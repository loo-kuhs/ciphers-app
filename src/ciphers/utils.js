...
export const ALPHAGRID = (() => {
  // Necesitamos crear una cuadricula de 6x6 de los caracteres alfanuméricos
  const GRID_SIZE = 8;
  let grid = new Array(GRID_SIZE);

  // Indice actual del arreglo alfanumérico
  let index = 0;
...
// MOD [Una solución para los problemas de JS con los residuos negativos]
...
// isNumber
// Revisa si hay números en la cadena
...
// Revisa si el carácter debe ser mayúscula
...
// Fuerza al carácter a ser compatible con el mayus dado de charToMatch
...
// Recorre un string determinado y llama a la función con el
// indice actual, la letra, y si esa letra es mayúscula
...
export function parseCipherArgs(args, defaults) {
  ...
  let isEncoding =
    args && typeof args.isEncoding === "boolean"
      ? args.isEncoding
      : defaults.isEncoding;

  let inputStr =
    args && typeof args.inputStr === "string"
      ? args.inputStr
      : defaults.inputStr;
