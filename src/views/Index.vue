<template>
  <div class="index">
    ...
    >
      ...
      <section class="ciphers" aria-label="Cipher Results">
        <Cipher
      ...
      <transition name="toast-transition-up-" mode="out-in">
        <div role="alert" class="toast" v-show="toast.isVisible">
          {{ toast.message }}
        </div>
      </transition>
    </main>
    ...
    <transition name="modal-transition-up-" mode="out-in" appear>
      <router-view role="dialog" aria-modal="true" :key="$route.path">
      </router-view>
    </transition>
  </div>
</template>
  ...
  .toggle {
    ...
    &__symbol {
      ...
      background-image: linear-gradient(
        90deg,
        $color__toggle--encode 0%,
        $color__toggle--decode 100%
      );
